...
def load_model(optimization_object):
    """
    Load the topic model for the resume of the optimiza

    :param optimization_object: dictionary of optimiza attributes saved in the jaon file
...
def select_metric(metric_parameters, metric_name):
    """
    Select the metric for the resume of the optimiza
...
def early_condition(values, n_stop, n_random):
    """
    Compute the early-stop criterium to stop or not the optimiza.
...
def check_instance(obj):
    ...
    :param obj: an object of the optimiza to be saved
